...
const FormInput = () => {
  ...
  return (
    <div className={`absolute  top-32 right-0 z-20 p-4 ${nightMode?' bg-black/30 border-gray-600':' bg-white/30 text-gray-900 border-gray-200'} backdrop-blur-xl border-2  rounded-l-xl`}>
      {/* <Select
...
<div>
  {allRegions.map((region)=>(
    <div 
    className={`cursor-pointer px-3 py-2 rounded-md flex font-medium text-xl gap-x-3 hover:bg-black/10 ${region.name==selectedRegion?.name?(nightMode?'bg-black/10 backdrop-blur-3xl':'bg-white/10 backdrop-blur-3xl' ):''}`} 
